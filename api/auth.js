...
function trialMeta(user) {
  ...
  const freeUntil = new Date(created.getTime() + FREE_TRIAL_DAYS * 86400000);
  const now = new Date();
...
async function currentUser(req) {
  ...
  const rows=await sbJson(`users?id=eq.${encodeURIComponent(id)}&select=id,email,name,username,plan,pro_until,created_at,last_seen_at,blocked,is_admin&limit=1`);
