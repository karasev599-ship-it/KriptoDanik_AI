...
async function currentUser(req) { if(!secret())return null; const token=parseCookies(req)[COOKIE];if(!token)return null;const [enc,sig]=token.split('.');if(!enc||!sig)return null;let payload;try{payload=Buffer.from(enc,'base64url').toString('utf8')}catch{return null}if(sign(payload)!==sig)return null;const [id,exp]=payload.split('.');if(!id||Number(exp)<Math.floor(Date.now()/1000))return null;const rows=await sbJson(`users?id=eq.${encodeURIComponent(id)}&select=id,email,name,username,plan,pro_until,created_at,last_seen_at,blocked,is_admin&limit=1`);const user=Array.isArray(rows)?rows[0]:null;if(!user||user.blocked)return null;return user;}
...
export default async function handler(req,res){
 ...
 try{
  ...
  if(req.method==='POST'&&action==='signup'){
   ...
   const rows=await sbJson('users',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({email,password_hash:hashPassword(password),name,username:username||email.split('@')[0],plan:'free'})});const user=rows?.[0];if(!user)throw new Error('User creation failed.');setSession(res,makeSession(user.id));return json(res,201,{authenticated:true,user});
  }
  if(req.method==='POST'&&action==='login'){
   const body=typeof req.body==='string'?JSON.parse(req.body):(req.body||{});const email=String(body.email||'').trim().toLowerCase(),password=String(body.password||'');const rows=await sbJson(`users?email=eq.${encodeURIComponent(email)}&select=id,email,password_hash,name,username,plan,pro_until,created_at,last_seen_at,blocked,is_admin&limit=1`);const user=rows?.[0];if(!user||user.blocked||!verifyPassword(password,user.password_hash))return json(res,401,{error:'Неверный email или пароль.'});await sbJson(`users?id=eq.${encodeURIComponent(user.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({last_seen_at:new Date().toISOString()})});delete user.password_hash;setSession(res,makeSession(user.id));return json(res,200,{authenticated:true,user});
  }
