(() => {
  ...
  function injectPlanInfo(){
    ...
    if(submit) signupForm.insertBefore(box,submit); else signupForm.appendChild(box);
    const style=document.createElement('style');
  ...
  function syncAppProfile(u){
    try{
      ...
      App.userData = {...(App.userData || {}),name:identity,username:u.username || identity,email:u.email || '',plan:planLabel(u),memberSince:u.created_at ? new Date(u.created_at).toLocaleDateString('ru-RU') : (App.userData?.memberSince || ''),trialDaysLeft:u.trial_days_left,freeUntil:u.free_until};
      if(!App.onboardingDone && App.onboardingOverlay){
        ...
        App.onboardingOverlay.classList.add('active'); App.saveState?.();
      } else { App.applyUserData?.(); }
  ...
  function open(){
    ...
    if(me){
      loginForm.hidden=true;signupForm.hidden=true;accountPanel.hidden=false;
      $('kdAuthTitle').textContent='Твой аккаунт';
      $('kdPanelPlan').textContent=planLabel(me);
      $('kdPanelUntil').textContent=planDetail(me);
      const up=$('kdUpgradeButton'); if(up){up.hidden=!!me.pro_active;up.textContent=me.trial_active?'Перейти на PRO · 4,99 $':'Подключить PRO · 4,99 $';}
    }else{loginForm.hidden=false;signupForm.hidden=true;accountPanel.hidden=true;}
  }
  function closeModal(){overlay.hidden=true;document.body.classList.remove('kd-auth-open');}
  function update(u){
    me=u||null;
    const name=u?.username||u?.name||u?.email?.split('@')[0]||'Гость';
    $('kdAccountName').textContent=name;
    $('kdAccountPlan').textContent=planLabel(u);
    $('kdAccountAvatar').textContent=name[0].toUpperCase();
    $('kdAccountAvatar').classList.toggle('pro',u?.pro_active);
    const badge=document.getElementById('settingsPlan');if(badge){badge.textContent=planLabel(u);badge.classList.toggle('badge-pro',u?.pro_active);}
    const p=document.getElementById('settingsEmail');if(p)p.textContent=u?.email||'—';
    const n=document.getElementById('settingsUsername');if(n)n.textContent=name;
    const ms=document.getElementById('settingsMemberSince');if(ms)ms.textContent=u?.created_at?new Date(u.created_at).toLocaleDateString('ru-RU'):'—';
    const av=document.getElementById('settingsAvatar');if(av)av.textContent=name[0].toUpperCase();
  ...
  $('kdUpgradeButton')?.addEventListener('click',()=>{error.textContent='PRO — 4,99 $/месяц. Оплата нужна для AI-инфраструктуры, поддержки AI Coach и дальнейшего развития проекта.';});
  injectPlanInfo();
